...
class XfmHfiveDataset:
            ...
            for k, h5 in enumerate(sorted_nicely(self.h5ls)):
                ...
                with h5py.File(self.dpath + h5) as h:
                    ...
                    for shot in range(d.shape[0]):
                        if shot % 100 == 0:
                            print(f'<atomize_reduce_h5> {self.group} {self.tag} {shot}/{d.shape[0]} frames generated')
                        if masked:
    ...
    def show_overview_figures(self,dump=True,show=True):
        self.mask = np.load(f'{self.apath}{self.tag}_mask.npy')
        ...
        #plt.clim(0,np.max(self.run_data_sum))
        if dump: plt.savefig(f'{self.apath}{self.tag}_sum.png')
        ...
        #plt.clim(0,np.max(self.run_data_avg))
        if dump: plt.savefig(f'{self.apath}{self.tag}_avg.png')
        # Reduced data
        ...
        plt.ylim([0.0, np.median(self.red_data_sum[:,1]) * 10])
        if dump: plt.savefig(f'{self.apath}{self.tag}_sum_red.png')
        ...
        plt.ylim([0.0, np.median(self.red_data_avg[:,1]) * 10])
        if dump: plt.savefig(f'{self.apath}{self.tag}_avg_red.png')
        if show:
            plt.show()
        plt.close('all')

    def quick_overview(self, run_limit=0, show=False, processdata=True,nproc=10,npt=2250):
        print(f'<fluxfm.overview> Analyzing run {self.tag}')
        if processdata==True:
              imagepath = self.scratch+"frames/"
              if os.path.isdir(imagepath) != True:
                   os.mkdir(imagepath)
              plotspath = self.scratch+"lineplots/" 
              if os.path.isdir(plotspath) != True:
                   os.mkdir(plotspath)
        ...
        for k, h5 in enumerate(self.h5ls[:run_limit]):
            ...
            with h5py.File(self.dpath + h5) as f:
                ...
                h5_sum_saxs = self.frm_integration(h5_sum * self.mask, npt=npt)
                np.savetxt(f'{self.apath}{self.tag}_sum_h5_{k + 1}_red.dat',
                           h5_sum_saxs)
                np.save(f'{self.apath}{self.tag}_sum_h5_{k + 1}_red.npy',
                        h5_sum_saxs)
                #save the png for the saxs profile of each h5 file
                plt.plot(h5_sum_saxs[:, 0], h5_sum_saxs[:, 1])
                plt.ylim([0.0, np.median(h5_sum_saxs[:,1]) * 10])
                plt.savefig(f'{self.apath}{self.tag}_sum_h5_{k + 1}_red.png')
                print(f'<fluxfm.quick_overview> {h5} shape:  {d.shape}')
                print(f'<fluxfm.quick_overview> {self.run_data_array.shape}')

        #
        # save npy and png file for some random frames 2D and 1D lineplot
        # generate all 1D lineplots and save them in a npy file
        # A.Martin July 2023
        #  
        if processdata == True:
               # print( d.shape, self.run_data_array.shape)
               d = self.run_data_array   #should really replace all the 'd's to avoid confusion
               # save random frames
               nflist =[]
               for i in np.arange(nproc):
                    nf = int(np.random.rand()*d.shape[0])
                    #print("nf max(d[nf,])", nf, np.max(d[nf,:,:]*self.mask), np.max(self.mask))
                    if nf>(d.shape[0]-1): nf = d.shape[0]-1
                    nflist.append(nf)
                    np.save(imagepath+f'{self.tag}_frame'+str(nf)+'.npy', d[nf,:,:]*self.mask)
                    plt.figure()
                    plt.imshow(d[nf,:,:].astype(np.float)*self.mask)
                    #if i==0:plt.imshow(self.mask)
                    #plt.clim([0,1])
                    plt.title('frame'+str(nf))
                    # plt.clim(0, np.median(d[nf,:,:]*self.mask) * 3)
                    plt.clim(0,1)
                    plt.colorbar()
                    plt.savefig(imagepath+f'{self.tag}_2D_frame'+str(nf)+'.png')
                    plt.close() 
               print("<overview> "+str(nproc)+" random 2D frames have been saved")


               #generate 1D plots
               #plots = np.zeros( (d.shape[0],npt,2) )
               #for i in np.arange(d.shape[0]):
               #   plots[i,:,:] =  self.frm_integration(d[i,:,:] * self.mask, npt=npt)
               #   np.save(plotspath+f'{self.tag}_all_qlineplots.npy', plots)
               #print("<overview all lineplots generated")
             
               for nf in nflist:
                  pl = self.frm_integration(d[nf,:,:] * self.mask, npt=npt)
                  np.save(plotspath+f'{self.tag}_1D_frame'+str(nf)+'.npy', pl) #plots[nf,:,:])
                  plt.figure()
                  plt.title(f'{self.tag} Reduced masked avg')
                  plt.xlabel('q / nm^{-1}')
                  plt.ylabel('Intensity / arb. units')
                  #plt.plot(plots[nf,:,0], plots[nf,:,1])
                  #plt.ylim([0.0, np.median(plots[nf,:,1]) * 10])
                  plt.plot(pl[:,0], pl[:,1])
                  #plt.ylim([0.0, np.median(pl[:,1]) * 10])
                  plt.savefig(plotspath+f'{self.tag}_qlineplot_'+str(nf)+'.png')
                  plt.close()
               print("random lineplots plotted")
        ...
        np.save(f'{self.apath}{self.tag}_sum_red.npy', self.frm_integration(self.run_data_sum * self.mask, npt=npt))
        ...
        np.save(f'{self.apath}{self.tag}_avg_red.npy', self.frm_integration(self.run_data_avg * self.mask, npt=npt))
        np.save(f'{self.apath}{self.tag}_avg_red_diff.npy',self.frm_integration(np.abs(diff_data_avg) * self.mask, npt=npt))
        if show:
            plt.figure(f'{self.tag} Masked sum')
            plt.imshow(self.run_data_sum * self.mask)
            plt.clim(0, np.median(self.run_data_sum) * 3)
