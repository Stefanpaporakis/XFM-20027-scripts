...
class angular_correlation:
    ...
    #
    def polarplot_angular_intershell_correlation( self, polar, polar2=None, realout=True, qbmin=-1,qbmax=-1):
        """
        ...
        if qbmin<0: qbmin=0
        if qbmax>polar.shape[0]: qbmax=polar.shape[0]
        ...
        out = np.zeros( (qbmax-qbmin,qbmax-qbmin,polar.shape[1]) , dtype=np.complex)
        for i in np.arange(qbmin,qbmax):
            for j in np.arange(qbmin,qbmax):
                out[i-qbmin,j-qbmin,:] = fpolar[i,:]*fpolar2[j,:].conjugate()
