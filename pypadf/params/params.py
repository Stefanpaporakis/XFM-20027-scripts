...
class params:
    ...
    #
    # Read config file and commandline
    #
    def parse_all_parameters(self):
        """Read config file, then parse commandline parameters; convert the path formats
        """
    ...
    def parse_config_file( self, fname ):
        ...
        print('pa: finished parsing config file')
    ...
    def parse_commandline_args( self ):
        ...
        for k in self.args.__dict__.keys():
            #print( "debug commandline reading", k, self.args.__dict__[k] )
            if k in self.d.keys():
                        if self.args.__dict__[k] is not None:
                            v = self.args.__dict__[k][0]
                        else:
                            continue
                        #print("debug commandline value", v, type(self.d[k].value))
