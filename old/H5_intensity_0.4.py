...
def peak_info(d):
        ...
        peaks, properties = find_peaks(one_d_slice[:,1])
        if peaks.size>0:
        ...
        return max_peak_height, max_peak_position
...
for i in files:
    ...
    for j in range(d1.shape[0]):
        ...
        max_peak_height, max_peak_positions = peak_info(d1[j,:,:])
        #dintegral_plot = peak_info(d1[j,:,:])
        d_list.append(max_peak_positions)
        
pathlib.Path(analysis_path+'/mapping_stuff').mkdir(parents = True,exist_ok = True)
np.save(analysis_path+'/mapping_stuff/'+'radial_peak_position',np.array(d_list))
